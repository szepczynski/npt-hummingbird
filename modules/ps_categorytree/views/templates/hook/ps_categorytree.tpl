    ...
    {if $nodes|count}
      <ul class="{$componentName}__list" data-depth="{$depth|escape:'htmlall':'UTF-8'}">
        {foreach from=$nodes item=node name="categories"}
          <li class="{$componentName}__item {if $node.children}accordion-item{/if} {if $node.in_path}active{/if}">
            ...
            {if $node.children}
              <div class="accordion-collapse collapse {if $node.in_path}show{/if}" id="category-tree-{$node.id|escape:'htmlall':'UTF-8'}">
